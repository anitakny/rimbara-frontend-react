...
function UserSearchDropdown({ excluded, onSelect, onClose }) {
  ...
  const lookup = async () => {
    ...
    // Auto-format: add dashes if missing
    ...
    <div data-dropdown className="absolute z-50 top-full left-0 mt-2 w-80 bg-white rounded-card border border-sand shadow-elevated overflow-hidden">
      ...
      {found && (
        <div className="px-3 py-2.5 animate-in fade-in slide-in-from-top-1 duration-300">
          ...
          <button type="button" onClick={confirm} className="w-full py-2 rounded-lg bg-forest text-white font-sans text-sm font-medium hover:bg-forest/90 transition-colors duration-[200ms] active:scale-[0.98]">
            Tambahkan
          </button>
        </div>
      )}

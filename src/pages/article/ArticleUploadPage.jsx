...
export default function ArticleUploadPage() {
  ...
  const [activeAction, setActiveAction] = useState('')    // 'review' | 'draft'
  ...
  const doSubmit = async (action) => {
    ...
    setSubmitting(true); setActiveAction(action); setFieldErrors({}); setServerError('')
    ...
    if (articleId) {
      ...
      if (!patch.ok) {
        ...
        setSubmitting(false); setActiveAction(''); return
      }
    } else {
    ...
    setSubmitting(false); setActiveAction('')
  }

  const canSubmit = !!file && !!thumbnail && !submitting && !parsing
    ...
    <div className="min-h-screen bg-bone">
        ...
        <div className="max-w-2xl mx-auto px-4 md:px-6 py-8 md:py-12">
          ...
          <div className="mb-8">
            ...
            <h1 className="font-serif text-h1 font-semibold text-ink leading-tight">
              Tulis <em className="font-accent italic text-clay">{label} Baru</em>
            </h1>
          ...
          <form onSubmit={e => e.preventDefault()} className="flex flex-col gap-6">
            ...
            <div className="bg-white rounded-card border border-sand shadow-subtle overflow-hidden">
              <div className="px-6 pt-5 pb-4 border-b border-sand">
                ...
                <p className="font-sans text-caption text-ash mt-0.5">
                  Gambar cover {label.toLowerCase()} (JPG, PNG, WebP — maks 5MB). Wajib diisi.
                </p>
              </div>
              <div className="p-6">
                {thumbPreview ? (
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-sans text-sm font-medium text-ink truncate mb-1">{thumbnail?.name}</p>
                      <p className="font-sans text-caption text-moss mb-3">
                        {thumbnail ? `${(thumbnail.size / (1024 * 1024)).toFixed(1)} MB` : ''}
                      </p>
                ...
                {/* Warn when file is selected but thumbnail is still missing */}
                {file && !thumbnail && (
                  <p className="font-sans text-caption text-clay mt-2 flex items-center gap-1.5">
                    <AlertCircle size={12} className="flex-shrink-0" />
                    Thumbnail wajib diunggah sebelum mengajukan {label.toLowerCase()}.
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="flex flex-col gap-2">
              <button
                ...
                {submitting && activeAction === 'review'
      ...
      {confirmOpen && (
        <div
          ...
          onKeyDown={e => e.key === 'Escape' && !submitting && setConfirmOpen(false)}
          tabIndex={-1}
        >
          <div className="bg-white rounded-card border border-sand shadow-elevated w-full max-w-sm">
            <div className="p-6">
              <h2 className="font-serif text-h2 font-semibold text-ink mb-1">Ajukan {label}?</h2>
              <p className="font-sans text-caption text-ash mb-5">
                {label} akan dikirim ke antrian review editor dan tidak dapat diedit selama proses review.
              </p>

              {/* Article preview */}
              <div className="flex items-center gap-3 p-3 bg-bone rounded-lg border border-sand mb-6">
                {thumbPreview ? (
                  <img
                    src={thumbPreview}
                    alt="thumbnail"
                  ...
                  <div className="w-14 h-10 rounded flex-shrink-0 bg-forest/10 flex items-center justify-center">
                    <FileText size={16} className="text-forest/40" />
                  </div>
                )}
                <div className="min-w-0">
                  <p className="font-sans text-sm font-medium text-ink leading-snug truncate">
                    {title.trim() || <span className="italic text-ash/60">Judul belum diisi</span>}
                  </p>
                  <p className="font-sans text-caption text-ash truncate">{file?.name}</p>
                </div>
              </div>
              ...
              <div className="flex gap-2">
                ...
                <button
                  type="button"
                  onClick={() => doSubmit('review')}
                  disabled={submitting}
                  className="flex-1 py-2.5 rounded-lg bg-forest text-bone font-sans text-sm font-medium
                    hover:bg-forest/90 disabled:opacity-50
                    transition-all duration-[240ms] flex items-center justify-center gap-2"
                >
                  {submitting
                    ? <><Loader2 size={13} className="animate-spin" />Mengajukan…</>
                    : 'Konfirmasi & Ajukan'
                  }
                </button>
              </div>

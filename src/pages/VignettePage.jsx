...
export default function VignettePage() {
  ...
  const loadQueue = async () => {
    ...
    if (ok) {
      ...
      setArticles(all.filter(a => a.content_type === 'VIGNETTE'))
    }
    setLoading(false)
  }
  ...
  const loadHistory = async () => {
    ...
    if (ok) {
      const all = Array.isArray(data) ? data : (data.results ?? [])
      setHistory(all.filter(a => ['PUBLISHED', 'REVISION', 'REJECTED'].includes(a.status) && a.content_type === 'VIGNETTE'))
    }
    setHistLoaded(true)
  ...
  const handleReview = async (articleId, action, noteOverride) => {
    ...
    if (ok) {
      ...
      setSuccessMsg(`Vignette berhasil ${labels[action] ?? 'diproses'}.`)
  ...
  return (
    <div className="min-h-screen bg-bone">
            ...
            <div className="flex items-end justify-between gap-4 flex-wrap">
              <div>
                <h1 className="font-serif text-h1 font-semibold text-ink leading-tight">
                  Antrian{' '}
                  <em className="font-accent italic text-clay">Review Vignette</em>
                </h1>
                <p className="font-sans text-body text-ash mt-1">
                  Klaim vignette, tinjau isinya, lalu berikan keputusan editorial.
                </p>
              </div>
              <button
                onClick={loadQueue}
              ...
              {loading ? (
                ...
                <div className="bg-white rounded-card border border-sand shadow-subtle px-8 py-16 text-center">
                  ...
                  <p className="font-sans text-body text-ash">
                    Tidak ada vignette yang menunggu review saat ini.
                  </p>
                </div>
              ) : (
                <div className="flex flex-col gap-3">
                ...
                <>
                  ...
                  {history.length === 0 ? (
                    <div className="bg-white rounded-card border border-sand shadow-subtle px-8 py-12 text-center">
                      ...
                      <p className="font-sans text-body text-ash">Vignette yang sudah diproses akan muncul di sini.</p>
                    </div>
                  ) : (
                    <div className="flex flex-col gap-3">
                      {history.map(article => {
      ...
      {confirmAction && (() => {
        ...
        const ACTION_CFG = {
          PUBLISH:  { heading: 'Terbitkan Vignette?',  btn: 'Terbitkan',    btnCls: 'bg-moss text-bone hover:bg-moss/90' },
          REVISION: { heading: 'Minta Revisi?',       btn: 'Minta Revisi', btnCls: 'bg-clay/15 text-clay border border-clay/30 hover:bg-clay/25' },
          REJECT:   { heading: 'Tolak Vignette?',      btn: 'Tolak',        btnCls: 'bg-sand text-ash border border-sand/80 hover:bg-ash/10 hover:text-ink' },
        }
                ...
                <div>
                  ...
                  {action === 'PUBLISH' && (
                    <p className="font-sans text-caption text-ash">
                      Vignette akan diterbitkan dan dapat diakses oleh publik.
                    </p>
                  )}
                  {action === 'REVISION' && (
                    <p className="font-sans text-caption text-ash">
                      Vignette akan dikembalikan ke penulis dengan catatan revisi.
                    </p>
                  )}
                  {action === 'REJECT' && (
                    <p className="font-sans text-caption text-ash">
                      Vignette akan ditolak dan penulis akan menerima pemberitahuan.
                    </p>
                  )}
                </div>
